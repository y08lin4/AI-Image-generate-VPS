...
export async function checkServerPassword(accessPassword: string): Promise<{ ok: boolean; message: string }> {
  const response = await fetch('/api/health', {
    headers: { 'X-Access-Password': accessPassword },
  })
  if (response.ok) return { ok: true, message: '服务端密码验证通过' }
  const data = await response.json().catch(() => null) as { message?: string } | null
...
function normalizeBaseUrlForBrowser(value: string) {
  ...
  if (window.location.protocol === 'https:' && url.protocol === 'http:') {
    throw new Error('浏览器直连模式下，HTTPS 页面不能请求 HTTP API；请改用服务端代理或 HTTPS API')
  }
...
function formatFetchError(message: string) {
  if (/abort|timeout|operation was aborted/i.test(message)) {
    return '请求超时：生图通常需要 100-300 秒，请调高超时时间或改用服务端流式代理'
  }
  if (/524|cloudflare/i.test(message)) return formatCloudflare524Error()
  if (/cors/i.test(message)) {
    return 'CORS：浏览器直连被上游拦截，建议切换到服务端流式代理模式'
  }
  if (/failed to fetch|load failed|networkerror/i.test(message)) {
    return '浏览器直连失败，可能是 CORS 或网络限制；建议切换到服务端流式代理模式'
  }
  return message || '请求失败'
}
